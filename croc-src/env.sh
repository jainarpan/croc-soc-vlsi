...
if [[ -d "${CROC_ROOT}/technology" ]]; then
    ...
    echo "[INFO][ENV] Init tech from container pre-installed PDK"
    ...
    export PDK_DIR_GDS_BOND="$CROC_ROOT/ihp13/bondpad/gds"
    echo "[INFO][ENV] PDK patches not needed (using container PDK)"

elif [[ -d "${CROC_ROOT}/ihp13/pdk/ihp-sg13g2" ]]; then
